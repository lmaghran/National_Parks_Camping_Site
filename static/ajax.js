    ...
    ///Adds campgrounds to the dom with links
      ...
      Object.keys(result).forEach(function(item) {
        ...
        //   // }
          ...
          let campground = $("<p id='replacement'><strong>" + String(item) +'</strong></p>')
          $('#replacement').append(campground);

          let emptyCampsite = $('<p>' + "No campsites available for these dates at this campground" +'</p>' +"<hl>")
...
$('#np-info').on('click',  (evt) => {
  ...
  $.ajax( {
    ...
    let image = $("<img class='image' src='"+ String(result['images'][0]['url']) + "' width=525px>" +
                "<p> " + String(result['images'][0]['caption']) + "</p>")
      $('#images').append(image);
    }
    });
  });

//Map options (include JSON for custom map styling)
var options = {
    zoom: 4.5,
